...
def calculate_coordinates(nivel):
    ...
    centro_x = (100 + 500) / 2
    centro_y = (100 + 500) / 2
    inicio_x = centro_x + 150 * sin(radians(nivel))
    inicio_y = centro_y - 150 * cos(radians(nivel))
    final_x = centro_x + 150 * sin(radians(nivel + 8))
    final_y = centro_y - 150 * cos(radians(nivel + 8))
...
def draw_progress_bar(inicio_x, inicio_y, final_x, final_y):
    ...
    canvas.create_line(inicio_x, inicio_y, final_x, final_y,
                       fill='deep sky blue', width=20)  # Aumentar el ancho de la línea

def draw_circles_and_text(centro_x, centro_y):
    # Dibuja los círculos y el texto en el lienzo
    canvas.create_oval(150, 150, 450, 450, fill='', outline='dark violet', width=5)  # Mantener el ancho original del borde
    canvas.create_oval(180, 180, 420, 420, fill='gray22', outline='dark violet', width=5)  # Mantener el ancho original del borde
...
def update_progress_bar():
    ...
    if tiempo_transcurrido < duracion:
        ...
        ventana.after(10, update_progress_bar)  # Actualizar más frecuentemente
    else:
        nivel = 359
        canvas.create_oval(100, 100, 500, 500, fill="", outline='', width=5)
